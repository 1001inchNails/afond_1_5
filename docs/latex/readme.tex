...
\section{Abrir tunel fuera de la red}
\hyperlink{anchor-indice}{\textbf{Volver}}\\
...
\verb|ngrok http 80 --host-header=l0wcs0g0wcscg0occc8gg8cw.192.168.0.66.sslip.io|

Con el enlace resultante podemos ver la API en cualquier dispositivo.

\clearpage

\section{Auto-desplegar con webhook de Coolify}
\hyperlink{anchor-indice}{\textbf{Volver}}\\

Para que la aplicación se auto despliegue cada vez que se hace un push al repositorio de github de la API.

\begin{figure}[h!]
    \centering
    \includegraphics[width=0.9\textwidth]{43.png}
    \caption{Activar auto deploy en la configuración avanzada del proyecto (Coolify dashboard)
}
\end{figure}
\FloatBarrier
...
\begin{figure}[h!]
    ...
    \caption{Ir a la pestaña de Webhooks, crear un secret y ponerselo.}
\end{figure}
\FloatBarrier

\clearpage
Ir a la pestaña Settings del repositorio de nuestra API, clickea Webhooks.

En payload URL introduce el Manual Git Webhook de Github, pero debes sustituir el dominio anterior a \verb|/webhooks| por el dominio que nos da ngrok después de introducir el comando \verb|ngrok http 8000|.

Ponemos el mismo secret que pusimos en la configuración de Coolify

\begin{figure}[h!]
    \centering
    \includegraphics[width=1\textwidth]{45.png}
    \caption{Comprobamos que haga el ping correctamente}
\end{figure}
\FloatBarrier
...
Ahora comprobamos con el webhook de Coolify \verb|http://l0wcs0g0wcscg0occc8gg8cw.192.168.0.68.sslip.io| el estado actual de la API
\begin{figure}[h!]
    \centering
...
Hacemos un cambio menor a nuestra API.

\begin{figure}[h!]
    \centering
    \includegraphics[width=1\textwidth]{48.png}
    \caption{Cambio menor 'QWERTY'}
\end{figure}
\FloatBarrier
...
\begin{figure}[h!]
    ...
    \caption{Comprobamos en Github que el push se haya detectado correctamente}
\end{figure}
\FloatBarrier
...
\begin{figure}[h!]
    ...
    \caption{Volvemos al navegador y F5 para comprobar que el auto deploy se ha ejecutado correctamente
}
\end{figure}
\FloatBarrier
...
La dirección IP de la máquina virtual cambia entre sesiones, de manera que hay que comprobarla y poner la correcta en el webhook proporcionado por Coolify.
\clearpage
...
Documentacion
\url{https://github.com/1001inchNails/afond_1_5}\\

API de prueba
\url{https://github.com/1001inchNails/afond_1-4}\\

Referencia Auto-Deploy
\url{https://coolify.io/docs/applications/ci-cd/github/auto-deploy}\\
